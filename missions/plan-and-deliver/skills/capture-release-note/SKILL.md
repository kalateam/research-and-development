...
| `writeCenterUnreleased` | no | Default `false` — set `true` when this dispatch edited R&D center and an center unreleased write is desired |
| `centerRoot` | no | Absolute R&D center checkout for optional write / center commit collection |
...
From **`HOSTING_ROOT`** (and optionally **`centerRoot`** when provided or when R&D center commits are clearly part of this dispatch):
...
2. Otherwise collect `git -C <root> log --oneline <baseRef>..HEAD` for:
   ...
   - R&D center checkout when `centerRoot` is set or R&D center commits are clearly part of this dispatch.
...
3. Intended write paths (hosting required; R&D center when `writeCenterUnreleased`)
...
| `centerFragmentPath` | string | Optional absolute R&D center unreleased path |
...
| Land skill under builtin-center / `.sedea/centers/sedea/skills/` | research-and-development plan-and-deliver path only |
